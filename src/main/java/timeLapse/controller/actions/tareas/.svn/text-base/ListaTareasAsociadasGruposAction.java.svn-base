...
/**
 * Action utilizado para listar las tareas vinculadas a grupos y que se quieren desvincular de ellos.
 * Devuelve solo tareas creadas por el usuario logueado.
 * @author robe
 * 
 */
@SuppressWarnings("serial")
@Service("listaTareasAsociadasGruposAction")
@Scope("request")
public class ListaTareasAsociadasGruposAction extends ListaTareasActionTemplate {
	@Resource
	ITareaBO neg;
	...
	public ListaTareasAsociadasGruposAction() {
	...
	public String execute() throws Exception {
		...
		/*
		 * Recojo los valores de los grupos.
		 */
		List<String> gruposAsociados = new ArrayList<String>();
		if(categorias != null){
			/*
			 * Añado todos los valores de los grupos asociados
			 */			
			for(String categoiaIterada : categorias){
				if(!categoiaIterada.equals(VALOR_TODAS_EN_LISTAS) 
					...
					gruposAsociados.add(categoiaIterada);
				}				
			}
			/*
			...
			if(gruposAsociados.isEmpty()){
				gruposAsociados.add(VALOR_ASOCIADAS_EN_LISTAS);				
			}
			
			
		}		
		filtro.setGrupos(gruposAsociados);
